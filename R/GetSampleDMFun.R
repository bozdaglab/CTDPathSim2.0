#'Sample-specific differentially methylated (DM) genes
#'
...
#' @param DNAmethylation_data A R dataframe with gene-centric DNA methylation beta values of bulk tumor samples. Rows are the genes, columns are the tumor samples.
...
#'
#' @return R dataframe objects each with a single column containing DM genes for each tumor sample. The column is labeled as gene.
#' @export
#'
#' @examples
GetSampleDMFun<-function(DNAmethylation_data,parallel=c('TRUE','FALSE'),ncores=2, CTDDirectory="~"){
  ...
  processFiles <- function(fileNames){
    ...
    save(pat_dm_genes,file=paste0(CTDDirectory,"/Patient_DM_genes/DM_Gene/",SampleName,"_pat_dm_genes.Rda"))
