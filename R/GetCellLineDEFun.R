#'Getting cell line-specific differentially expressed (DE) genes
#'
...
#' @param RnaSeq_data A R dataframe with gene-centric expression values of cell lines. Rows are the genes, columns are the tumor samples.
...
#'
#' @return R dataframe objects each with a single column containing DE genes for each cell line. The column is labeled as gene.
#' @export
#'
#' @examples
GetCellLineDEFun<-function(RnaSeq_data,parallel=c('TRUE','FALSE'),ncores=2, CTDDirectory="~"){
  library(matrixStats)
  library(parallel)
  library(pbapply)
  if((is(RnaSeq_data, "SummarizedExpirament"))){
    RnaSeq_data<-assay(RnaSeq_data)
  }

  dir.create(paste0(CTDDirectory,"/CellLine_DE_genes/"))
  dir.create(paste0(CTDDirectory,"/CellLine_DE_genes/UP_Gene/"))
  dir.create(paste0(CTDDirectory,"/CellLine_DE_genes/DOWN_Gene/"))
  dir.create(paste0(CTDDirectory,"/CellLine_DE_genes/DE_Gene/"))
  processFiles <- function(fileNames){
    ...
    save(upgene,file=paste0(CTDDirectory,"/CellLine_DE_genes/UP_Gene/",SampleName,"_upgene.Rda"))
    save(downgene,file=paste0(CTDDirectory,"/CellLine_DE_genes/DOWN_Gene/",SampleName,"_downgene.Rda"))
    save(pat_de_genes,file=paste0(CTDDirectory,"/CellLine_DE_genes/DE_Gene/",SampleName,"_cell_de_genes.Rda"))
